package com.adamatomic.flixel
{
	...
	public class FlxButton extends FlxCore
	{
		...
		//@param	TextOff		A FlxText object to use to display text on this button (optional)
		//@param	TextOn		A FlxText object that is used when the button is highlighted (optional)
		public function FlxButton(X:int,Y:int,Image:FlxSprite,Callback:Function,ImageOn:FlxSprite=null,TextOff:FlxText=null,TextOn:FlxText=null)
		{
			super();
			x = X;
			y = Y;
			_off = FlxG.state.add(Image) as FlxSprite;
			if(ImageOn == null) _on = _off;
			else _on = FlxG.state.add(ImageOn) as FlxSprite;
			width = _off.width;
			height = _off.height;
			if(TextOff != null) _offT = FlxG.state.add(TextOff) as FlxText;
			if(TextOn == null) _onT = _offT;
			else _onT = FlxG.state.add(TextOn) as FlxText;
