package com.adamatomic.flixel
{
	...
	public class FlxGame extends Sprite
	{
		...
		private var _logoTimerFinished:Number;
		...
		public function FlxGame(GameSizeX:uint,GameSizeY:uint,InitialState:Class,Zoom:uint=2,BGColor:Number=0xff000000,FlixelColor:Number=0xffffffff,FlixelSound:Class=null,Frame:Class=null,ScreenOffsetX:uint=0,ScreenOffsetY:uint=0)
		{
			_z = Zoom;
			_gx = ScreenOffsetX;
			_gy = ScreenOffsetY;
		...
		//@desc		This function is only used by the FlxGame class to do important internal management stuff
		private function onFocus(event:FocusEvent=null):void
		{
			flash.ui.Mouse.hide();
		...
		//@desc		This function is only used by the FlxGame class to do important internal management stuff
		private function onFocusLost(event:FocusEvent=null):void
		{
			flash.ui.Mouse.show();
		...
		private function onEnterFrame(event:Event):void
		{
			...
			if(_logoComplete)
			{
				...
				if(!_paused)
				{
					...
					if(_flash.visible)
					{
						_flash.alpha -= _elapsed/_flashDelay;
						if(_flash.alpha <= 0)
			...
			{
				if(root.loaderInfo.url.indexOf("localhost") >= 0)
				{
					_logoComplete = true;
					switchState(_iState);
				}
				else
				{
					...
					if(_logoTimer > _logoTimerFinished)
					{
			...
			else if(root != null)
			{
				...
				//This is some hacky bullshit that always needs to come last, to control focus tracking
				_focusField = new TextField();
				_focusField.width = FlxG.width*_z;
				_focusField.height = FlxG.height*_z;
				_focusField.selectable = false;
				addChild(_focusField);
				_focusField.addEventListener(FocusEvent.FOCUS_IN, onFocus);
				_focusField.addEventListener(FocusEvent.FOCUS_OUT, onFocusLost);
		...
		private function flash(Color:uint, Duration:Number=1):void
		{
			if(Color == 0)
			{
				_flash.visible = false;
		...
		private function fade(Color:uint, Duration:Number=1, FadeComplete:Function=null):void
		{
			if(Color == 0)
			{
				_fade.visible = false;
